package java2novice.ds.stack;
/*Convert a decimal into a binary number using stack.

This example shows how to convert decimal number into a binary 
number using stack.
*/
public class _6MyDecimalToBinary {
	 
    public static String convertDecialToBinary(int number){
         
        StringBuilder binary = new StringBuilder();
        MyDynamicStack_2 stack = new MyDynamicStack_2(10);
        if(number == 0){
            binary.append("0");
        } else {
            while(number != 0){
...
class MyDynamicStack_2 {
     ...
     * @param size
     */
    public MyDynamicStack_2(int size) {
        this.stackSize = size;
    ...
    public static void main(String[] args) {
        MyDynamicStack_2 stack = new MyDynamicStack_2(2);
        for(int i=1;i<10;i++){
            stack.push(i);
        }
        for(int i=1;i<4;i++){
