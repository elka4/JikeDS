...
// version 3: Quick Sort 2
    ...
    public ListNode sortList3(ListNode head) {
        ...
        ListNode left = sortList3(pair.first);
        ListNode right = sortList3(pair.second);

        getTail3(left).next = right; // O(n)
    ...
    // 1->2->3 return 2
    // 1->2 return 1
    private ListNode findMedian3(ListNode head) {
        ListNode slow = head, fast = head.next;
        while (fast != null && fast.next != null) {
            slow = slow.next;
            fast = fast.next.next;
    ...
    private ListNode getTail3(ListNode head) {
        if (head == null) {
            return null;
        }
