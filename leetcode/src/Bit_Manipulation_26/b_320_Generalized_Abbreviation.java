...
​n
​​ ). For each call to backtrack, it either returns without branching, or it branches into two recursive calls. All these recursive calls form a complete binary recursion tree with 2^n2
​n
​​  leaves and 2^n - 12
​n
​​ −1 inner nodes. For each leaf node, it needs O(n)O(n) time for converting builder to String; for each internal node, it needs only constant time. Thus, the total time complexity is dominated by the leaves. In total that is O(n2^n)O(n2
​n
​​ ).

Space complexity : O(n)O(n). If the return list doesn't count, we only need O(n)O(n) auxiliary space to store the characters in StringBuilder and the O(n)O(n) space used by system stack. In a recursive program, the space of system stack is linear to the maximum recursion depth which is nn in our problem.
 ...
 */
    ...
    Time complexity : O(n 2^n)O(n2
​n
​​ ). Building one abbreviation from the number xx, we need scan all the nn bits. Besides the StringBuilder::toString function is also linear. Thus, to generate all the 2^n2
​n
